...
int main(int argc, char** argv){
    ...
    while(ros::ok()){
        if(!msg_flag){
            ...
            val_msg.DATA[0] = (BYTE)(tar_steer*10 & 0xff);
            val_msg.DATA[1] = (BYTE)((tar_steer*10 & 0xff00)>>8);
            val_msg.DATA[2] = 100;
            val_msg.DATA[3] = (BYTE)((uint16_t)(reqMax*100) & 0xff);
            val_msg.DATA[4] = (BYTE)(((uint16_t)(reqMax*100) & 0xff00)>>8);
            msg_flag =false;
